...
class Tobin():
    ...
    def __init__(self, serial):
        ...
        self.discovery()
        self._logger.info("Tobin got %d devices in %d slaves." % (len(self._devices),len(self._slaves)))
        self._connect_to_broker()
        schedule.every(1).minutes.do(self.discovery)
        self.dump()

    def _connect_to_broker(self):
        self._logger.info("Tobin is connecting to the message broker.")
    ...
    def get_reading(self, device=False, id=False):
        if device:
            self._logger.debug('Tobin is reading device %s as %s at %s:%s'%(device.__class__, device.id, device._slave.id, device._address))
            response = device.get_reading()
            self._broker.publish("tobin/devices/" + str(device.id), response)
            return response
    ...
    def set_value(self, value, device=False, id=False):
        if device:
            try:
                self._logger.debug('Tobin is writing device %s as %s at %s:%s'%(device.__class__, device.id, device._slave.id, device._address))
    ...
    def start_poller(self):
        self._logger.info('Tobin is starting poller.')
    ...
    def _poller(self, stop_event):
        while (not stop_event.is_set()):
            ...
            for key, slave in self._slaves.items():
                self._logger.debug('Tobin poller is reading slave %s'%slave.id)
                ...
                try:
                    ...
                    for i in range(len(slave.nodes)):
                        ...
                        if device.got_news:
                            ...
                            self._broker.publish("tobin/devices/" + str(device.id), device_status, retain=True)
